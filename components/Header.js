import Link from "next/link"

export default function Header() {
  return (
    <header className="bg-gray-900 text-white shadow-md">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">

        {/* Logo */}
        <Link
          href="/"
          className="text-2xl font-bold tracking-wide hover:text-gray-300 transition"
        >
          FreshJobs Store
        </Link>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex space-x-6 text-sm items-center">
          <Link href="/" className="hover:text-gray-300 transition">
            Home
          </Link>

          {/* 🔹 Work From Home */}
          <Link
            href="/work-from-home"
            className="hover:text-gray-300 transition font-medium"
          >
            Work From Home
          </Link>

          {/* 🔹 High Paying WFH */}
          <Link
            href="/work-from-home/high-paying"
            className="hover:text-gray-300 transition font-medium text-green-400"
          >
            High Paying WFH
          </Link>

          {/* 🔹 AI Jobs */}
          <Link
            href="/ai-jobs"
            className="hover:text-gray-300 transition font-medium"
          >
            AI Jobs
          </Link>

          {/* 🔹 International Jobs (NEW) */}
          <Link
            href="/international-jobs"
            className="hover:text-gray-300 transition font-medium text-blue-400"
          >
            International Jobs
          </Link>

          <Link
            href="/government-jobs"
            className="hover:text-gray-300 transition"
          >
            Government Jobs
          </Link>

          <Link
            href="/job-sources"
            className="hover:text-gray-300 transition font-semibold text-yellow-300"
          >
            Job Sources
          </Link>

          <Link href="/resume-builder" className="hover:text-gray-300 transition">
            Resume Builder
          </Link>

          <Link href="/about" className="hover:text-gray-300 transition">
            About
          </Link>

          <Link href="/contact" className="hover:text-gray-300 transition">
            Contact
          </Link>

          <Link href="/privacy" className="hover:text-gray-300 transition">
            Privacy
          </Link>

          <Link href="/terms" className="hover:text-gray-300 transition">
            Terms
          </Link>
        </nav>

        {/* Mobile Menu Icon (later enhancement) */}
        <div className="md:hidden">
          <button className="focus:outline-none">
            <svg
              className="w-6 h-6"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M4 6h16M4 12h16M4 18h16"
              />
            </svg>
          </button>
        </div>
      </div>
    </header>
  )
}